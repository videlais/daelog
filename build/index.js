...
var b=this.attributes["data-passage"].value,c=window.daelog.findPassageByName(b),d=a.target.textContent,e=document.querySelector(".choices");// Find the passage by its name
e.innerHTML=d,e.className="",window.daelog.showPassage(c)})}// Append the child element to the HTML <body>
// (If it already exists, it will not be appended.)
}catch(a){f.e(a)}finally{f.f()}document.body.appendChild(b)}},{key:"parse",value:function parse(a){a=a.replace(/&lt;/g,function(){return"<"}),a=a.replace(/&gt;/g,function(){return">"}),a=a.replace(/\n/g,function(){return"<br>"}),a=a.replace(/#(.*?)#/g,function(a){return window.tracery.grammar.flatten(a)}),a=a.replace(/\[\[(.*?)\]\]/g,function(a,b){var c=b,d=b.indexOf("|");/* display|target format */if(-1!==d)c=b.substr(0,d),b=b.substr(d+1);else{/* display->target format */var e=b.indexOf("->");if(-1!==e)c=b.substr(0,e),b=b.substr(e+2);else{/* target<-display format */var f=b.indexOf("<-");-1!==f&&(c=b.substr(f+2),b=b.substr(0,f))}}return"<a href=\"javascript:void(0)\" data-passage=\""+b+"\">"+c+"</a>"});// Create a DocumentFragment
var b=new DocumentFragment,c=document.createElement("body");// Create a BODY
c.innerHTML=a,b.append(c);var d,e="",f=b.querySelectorAll("a"),g=[],h=_createForOfIteratorHelper(f);try{// Clone the previous links into a new array
for(h.s();!(d=h.n()).done;){var r=d.value;g.push(r.cloneNode(!0)),r.remove()}// Find all CHARACTER elements
}catch(a){h.e(a)}finally{h.f()}var i,j=b.querySelectorAll("character"),k=_createForOfIteratorHelper(j);try{for(k.s();!(i=k.n()).done;){var m=i.value,n=m.attributes.name.value,o=m.textContent;if("me"===n){e+="\n          <div class=\"container darker\">\n            <p>".concat(o,"</p>\n            <div class=\"choices\">");var p,q=_createForOfIteratorHelper(g);try{for(q.s();!(p=q.n()).done;){var l=p.value;e+="\n            ".concat(l.outerHTML,"\n            <br>")}}catch(a){q.e(a)}finally{q.f()}e+="</div>\n          </div>"}else{var s=window.tracery.grammar.flatten("#".concat(n,"#"));e+="\n          <div class=\"container\">\n            <p><strong>".concat(n,"</strong>: ").concat(s,"</p>\n          </div>")}m.remove()}}catch(a){k.e(a)}finally{k.f()}return a+=e,a=a.replace(/<character(.*?)>(.*?)<\/character>/g,function(){// Return an empty string
return""}),a}}]),a}(),_default=Daelog;exports["default"]=_default;

},{}],3:[function(require,module,exports){
"use strict";var _traceryGrammar=_interopRequireDefault(require("tracery-grammar")),_Daelog=_interopRequireDefault(require("./Daelog.js"));function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}window.tracery=_traceryGrammar["default"],window.variables={},window.daelog=new _Daelog["default"];

},{"./Daelog.js":2,"tracery-grammar":1}]},{},[3]);
